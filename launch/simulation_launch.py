...
def generate_launch_description():
    ...
    evaluation_manager = Node(
        package='riemannian_motion_policy',  # Adjust package name if different
        executable='evaluation_manager.py',
    ...
    return LaunchDescription([
        ...
        run_move_group_node,
        rviz_node,
        # Start evaluation after everything is ready
        RegisterEventHandler(
            OnProcessExit(
                target_action=start_controller,
                on_exit=[evaluation_manager],
            )
        ),
    ])